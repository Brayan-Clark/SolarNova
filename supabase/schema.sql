...
-- Inscriptions à la newsletter (footer)
...
-- Newsletter : inscription publique uniquement
...
create policy "newsletter_admin_read" on public.newsletter for select to authenticated using (has_permission('leads.view'));

-- 6) COMPATIBILITÉ UPSERT -----------------------------------
-- Les colonnes "id" doivent être "by default" (et non "always") pour que
-- l'upsert de Supabase fonctionne. Idempotent : sans effet si déjà le cas.
-- (Corrige aussi les bases créées avec une ancienne version du script.)
alter table public.reviews        alter column id set generated by default;
alter table public.contacts       alter column id set generated by default;
alter table public.articles       alter column id set generated by default;
alter table public.solutions      alter column id set generated by default;
alter table public.quotes         alter column id set generated by default;
alter table public.newsletter     alter column id set generated by default;
alter table public.app_roles      alter column id set generated by default;
alter table public.app_user_roles alter column id set generated by default;

-- 7) ANTI-VERROUILLAGE : protéger le DERNIER super administrateur ---
-- Empêche de supprimer ou de rétrograder le dernier compte super_admin,
-- y compris via un appel direct à l'API (la clé service_role déclenche
-- quand même les triggers). C'est le garde-fou côté base de données,
-- en complément de la protection dans l'interface.
...
declare
  super_count integer;
begin
  -- Nombre de super admins actuels.
  select count(*) into super_count
    from public.app_user_roles
    where role_name = 'super_admin';

  if (tg_op = 'DELETE') then
    if old.role_name = 'super_admin' and super_count <= 1 then
      raise exception 'Impossible de supprimer le dernier super administrateur.';
    end if;
    return old;
  elsif (tg_op = 'UPDATE') then
    if old.role_name = 'super_admin'
       and new.role_name <> 'super_admin'
       and super_count <= 1 then
      raise exception 'Impossible de rétrograder le dernier super administrateur.';
    end if;
    return new;
  end if;
  return new;
...
drop trigger if exists trg_protect_last_super_admin on public.app_user_roles;
