...
La connexion se fait par **lien magique** : on entre son email, on reçoit un
lien par email, on clique → connecté (aucun mot de passe à retenir).
...
- **En production (avec Supabase)** : entre ton email sur `/admin` → tu reçois un
  lien de connexion par email.

### ⚙️ Étape indispensable : autoriser l'URL de connexion
Pour que le lien magique te ramène bien sur le site, dans Supabase :
**Authentication → URL Configuration** :
- **Site URL** : `https://brayan-clark.github.io/SolarNova/admin`
- **Redirect URLs** : ajoute la même URL (`https://brayan-clark.github.io/SolarNova/admin`)
  et, pour tester en local, `http://localhost:4321/admin`.

> 📧 L'envoi d'emails intégré de Supabase est **limité** (quelques mails/heure sur
> le plan gratuit). Pour un usage réel, configure un SMTP (Authentication →
> Emails → SMTP) — ex. Resend, Brevo.
...
- Le super admin, dans les onglets **Rôles & accès** et **Utilisateurs** :
  ...
  2. **invite** une personne par son email + un rôle (onglet Utilisateurs → « Inviter »).
- Une personne **sans rôle attribué** voit « Accès non attribué ».

### Comment inviter ton équipe

Il n'y a **pas de page d'inscription publique**. Dans `/admin` → onglet
**Utilisateurs** → **Inviter** : tu saisis l'**email + le rôle**. La personne va
ensuite sur `/admin`, entre son email, reçoit le lien magique et se connecte —
elle obtient automatiquement le rôle que tu lui as donné. Aucun mot de passe, et
**rien à déployer**.

> 🔒 On ne peut pas retirer/rétrograder le **dernier** super administrateur
